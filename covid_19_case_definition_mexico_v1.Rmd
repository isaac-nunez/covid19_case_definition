...
pre_post_df_ssa_in <- group_by(base_cdmx_v1, by= fechreg) %>% 
  ...
  mutate(pre_test_prob = round(positivos/n, 2),
         ...
         definicion = "Mx suspected case in")
...
plot_positive_case <- ggplot(pre_post_df, aes(x=fechreg, y = rollmean(post_test_prob_pos, 7,
                                            align="right", fill =0), shape = definicion))+
  geom_point()+
...
#scale_linetype_manual(values=c("dotdash", "dotted", "longdash","twodash"))+
#geom_line(aes(y= rollmean(pre_test_prob, 7,
#                                            align="right", fill =0)),linetype= 1, size = 0.5)+
